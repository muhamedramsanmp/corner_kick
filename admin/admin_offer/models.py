    ...
    def clean(self):
        ...
        if not re.match(
            ...
            raise ValidationError({
                "offer_name":
                "Offer name can contain only letters, numbers and spaces."
        ...
        duplicate_offer = Offer.objects.filter(
            offer_name__iexact=self.offer_name,
            is_deleted=False
        ).exclude(
            id=self.id
        )
...
class CategoryOffer(models.Model):
    ...
    def current_status(self):
        ...
        if (
            self.offer.is_active and
            self.offer.start_date <= today <= self.offer.end_date
        ):
            return "Active"
