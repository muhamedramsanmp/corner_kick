...
def offer_management(request):
    ...
    if search:
        ...
        offers = offers.filter(Q(offer_name__icontains=search))

    if offer_type:

        offers = offers.filter(apply_to=offer_type)
    ...
    active_offers = Offer.objects.filter(
        is_deleted=False, is_active=True, start_date__lte=today, end_date__gte=today
    ).count()

    expired_offers = Offer.objects.filter(is_deleted=False, end_date__lt=today).count()
...
def add_offer(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if apply_to not in ["PRODUCT", "CATEGORY"]:

                messages.error(request, "Invalid offer type selected.")
            ...
            with transaction.atomic():
                ...
                if apply_to == "PRODUCT":
                    ...
                    CategoryOffer.objects.create(offer=offer, category_id=category_id)

            messages.success(request, "Offer created successfully.")

            return redirect("admin_offer:offer_management")
...
def edit_offer(request, offer_id):
    ...
    if request.method == "POST":
        ...
        if offer.apply_to == "PRODUCT":
            ...
            relation = OfferProduct.objects.filter(offer=offer).first()

            if relation:

                form_data["product"] = relation.product.id

        else:

            relation = CategoryOffer.objects.filter(offer=offer).first()

            if relation:

                form_data["category"] = relation.category.id
...
def delete_offer(request, offer_id):
    ...
    if request.method == "POST":
        ...
        Offer.objects.filter(id=offer_id, is_deleted=False).update(is_deleted=True)
